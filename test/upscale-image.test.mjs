...
test("AI Enhance sends GPT Image 2 with poster-aware framing and restrained detail", async () => {
  ...
  assert.equal(form.get("size"), "1008x1792");
  ...
  assert.match(form.get("prompt"), /lower 30 percent/);

  assert.equal(responseBody.engine, "gpt-image-2");
  assert.equal(responseBody.framing, "poster-aware");
  assert.equal(responseBody.aspect, "9:16");
  assert.equal(responseBody.outputSize, "1008x1792");
